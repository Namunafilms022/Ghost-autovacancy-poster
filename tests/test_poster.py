...
class TestThemes:
    def test_default_theme(self, poster):
        vid = create_vacancy()
        html = poster.generate(vid, theme='default')
        assert 'default.html' not in html

    def test_dark_theme(self, poster):
        vid = create_vacancy()
        html = poster.generate(vid, theme='dark')
...
class TestSave:
    ...
    def test_save_with_theme(self, poster):
        vid = create_vacancy()
        with tempfile.TemporaryDirectory() as d:
            path = poster.save(vid, os.path.join(d, 'poster.html'), theme='dark')
            content = path.read_text(encoding='utf-8')
...
class TestDatabaseRecord:
    def test_saves_vacancy_poster_record(self, poster):
        ...
        assert records[0].template_name == 'default.html'
        assert records[0].theme == 'default'
        session.close()

    def test_multiple_generations_create_multiple_records(self, poster):
        vid = create_vacancy()
        poster.generate(vid, theme='default')
        poster.generate(vid, theme='dark')
...
class TestPhoneReplacement:
    ...
    def test_no_phone_in_message(self, poster):
        vid = create_vacancy(raw_message='We are hiring!\nContact: email@test.com')
        html = poster.generate(vid)
        assert 'Not specified' in html or 'email@test.com' in html
